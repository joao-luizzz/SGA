...
from materials.services import atualizar_material, criar_material, excluir_material
...
class TestMaterialServices:
    ...
    def setup_ambiente(self, user_professor, user_aluno, user_coordenacao):
        ...
        disciplina = Disciplina.objects.create(
            codigo="DIR101", nome="Direito Civil", curso=curso, carga_horaria=60
        )
    ...
    def test_outro_professor_nao_pode_criar_material_em_turma_alheia(
        self, setup_ambiente
    ):
    ...
    def test_atualizar_material_substituindo_arquivo_por_link(
        self, setup_ambiente, django_capture_on_commit_callbacks
    ):
        turma, prof, _, _, _ = setup_ambiente
        arquivo = SimpleUploadedFile(
            "apostila_antiga.pdf", b"dados", content_type="application/pdf"
        )
        material = criar_material(
            turma=turma, autor=prof, titulo="Apostila", arquivo=arquivo
        )
    ...
    def test_atualizar_material_substituindo_arquivo_por_novo_arquivo(
        self, setup_ambiente, django_capture_on_commit_callbacks
    ):
        turma, prof, _, _, _ = setup_ambiente
        arquivo1 = SimpleUploadedFile(
            "v1.pdf", b"versao 1", content_type="application/pdf"
        )
        material = criar_material(
            turma=turma, autor=prof, titulo="Doc V1", arquivo=arquivo1
        )
        caminho1 = material.arquivo.path

        arquivo2 = SimpleUploadedFile(
            "v2.pdf", b"versao 2", content_type="application/pdf"
        )
        with django_capture_on_commit_callbacks(execute=True):
            atualizar_material(
                material=material, autor=prof, titulo="Doc V2", arquivo=arquivo2
            )
    ...
    def test_excluir_material_apaga_arquivo_do_disco(
        self, setup_ambiente, django_capture_on_commit_callbacks
    ):
        turma, prof, _, _, _ = setup_ambiente
        arquivo = SimpleUploadedFile(
            "apostila.pdf",
            b"conteudo apostila teste",
    ...
    def test_falha_no_salvamento_preserva_arquivo_anterior(
        self, setup_ambiente, monkeypatch
    ):
        turma, prof, _, _, _ = setup_ambiente
        material = criar_material(
            turma=turma,
        ...
        caminho_antigo = material.arquivo.path
        nome_antigo = material.arquivo.name
        storage = material.arquivo.storage

        save_original = MaterialAcademico.save

        def salvar_e_falhar(self, *args, **kwargs):
            save_original(self, *args, **kwargs)
            raise DatabaseError("falha de banco simulada")

        monkeypatch.setattr(MaterialAcademico, "save", salvar_e_falhar)
        ...
        with pytest.raises(DatabaseError):
            atualizar_material(
                ...
                arquivo=SimpleUploadedFile("nova.pdf", b"dados novos"),
            )

        nome_novo = material.arquivo.name

        assert nome_novo != nome_antigo
        assert storage.exists(nome_novo) is False
        assert storage.exists(nome_antigo) is True
        assert os.path.exists(caminho_antigo)
